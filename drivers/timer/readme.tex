...
\subsection*{Task 0: 1\,MHz or 8\,MHz? (10 min)}
Show the numbers 0 to 9 on the seven-segment display. Change the number every
second. Use \code{SevenSeg(digit)} and \code{\_delay\_ms(1000)}, and write
\code{\#define F\_CPU 8000000UL} at the top of the file.
\begin{enumerate}
  \item Upload with \code{make SRC=Task0.cpp}. This also sets the chip to the
        external 8\,MHz crystal. Check with a stopwatch: is one step one second?
  \item Now change the chip to the internal 1\,MHz clock with
        \code{make int-1mhz}. Do not upload the program again.
\end{enumerate}
What is the difference? How long is one ``second'' now? Explain why.
...
in \code{TIMER\_NORMAL} mode with prescaler 1024 for the timing (external
8\,MHz crystal). The LEDs are active-low, so write the inverted value: \code{GPIO.write(PORTB, \textasciitilde count)}.

One Timer0 overflow is too short for 100\,ms, so use 10 overflows of 10\,ms.
Calculate the preload value for 10\,ms. In the main loop, do these two steps
10 times:
\begin{enumerate}
...
Start \code{Timer0} in \code{TIMER\_CTC} mode with prescaler 1024 and
\code{OC\_TOGGLE}. The main loop only does one thing: it reads the switches
(\code{GPIO.read(PINC)}) and writes the value with \code{Timer0.setCompare()}.
You do not need to check any flag. At 8\,MHz, calculate the slowest possible
blink frequency. What do you see on D3? Explain why.
...
\begin{itemize}
  ...
  \item \textbf{Task 3.3 (Breathing LED with two timers):} LED D3 should slowly
        become brighter from 0\,\% to 100\,\%, and then slowly darker from
        100\,\% to 0\,\%. This repeats all the time.
\end{itemize}

\paragraph{Task 3.3 step by step.} You need two timers, and each timer has
one job:
\begin{itemize}
  \item \textbf{Timer0 makes the brightness.} It creates the PWM signal on D3,
        exactly as in Task 3.1 (Fast PWM, about 4\,kHz, \code{PWM\_INVERTING}).
  \item \textbf{Timer1 is a clock that ``ticks'' every 10\,ms.} Use
        \code{TIMER\_CTC} mode. Every 10\,ms there is a compare match, and
        \code{Timer1.compareMatched()} returns \code{true} one time.
\end{itemize}
Calculate the Timer1 compare value with $t = (\text{OCR} + 1) \cdot N / F_{\text{CPU}}$.
Hint: with prescaler $N = 64$ at 8\,MHz, one count takes $8\,\mu$s. How many
counts are 10\,ms?

On every tick, change the brightness by 1\,\%. Use two variables:
\code{percent} (the brightness now, 0--100) and \code{step} (\code{+1} for
brighter, \code{-1} for darker). When \code{percent} reaches 100, change
\code{step} to \code{-1}. When it reaches 0, change \code{step} to \code{+1}.
One full ``breath'' then takes $200 \times 10\,\text{ms} = 2$\,s.

\begin{lstlisting}
Timer0.begin(TIMER_FAST_PWM, ??, PWM_INVERTING);  // brightness (PWM on D3)
Timer1.begin(TIMER_CTC, 64);                      // tick every 10 ms
Timer1.setCompare(??);
...
while (true) {
  if (Timer1.compareMatched()) {   // true once every 10 ms
    percent = percent + step;
    if (percent == ??) step = ??;  // top reached: go darker
    if (percent == ??) step = ??;  // bottom reached: go brighter
    Timer0.setDutyCycle(percent);
  }
}
\end{lstlisting}
Why is this better than \code{\_delay\_ms(10)}? The CPU never waits, so the
loop can do other work at the same time (for example, read the buttons).

\paragraph{Expected result.} You can watch a recording of the finished task on
the board in the repository (click the link):\\
\href{https://github.com/AliSahafi/ATMEGA32A/blob/main/drivers/timer/task3_3.gif}{\code{drivers/timer/task3\_3.gif}}
